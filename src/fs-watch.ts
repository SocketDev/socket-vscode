...
export class SharedFilesystemWatcher {
  ...
  watch(partial: SharedFilesystemWatcherHandler): vscode.Disposable {
    ...
    if (partial.onDidChange) {
      handler.onDidChange = partial.onDidChange
    }
    if (partial.onDidCreate) {
      handler.onDidCreate = partial.onDidCreate
    }
    if (partial.onDidDelete) {
      handler.onDidDelete = partial.onDidDelete
    }
