...
from the environment", never the key itself and never a stand-in for it. If a
command's output would contain one, summarise the outcome rather than pasting
the output.
