 ...
 *
 * Reads `./toml.wasm` from disk synchronously at module-load time via
...
/**
 * Walk every key-value pair in the document. For each leaf or table key, invoke
 * `cb` with the dotted path and the surrounding spans. Equivalent to the old
 * `traverseTOMLKeys` helper in util.ts.
 *
 * Why we visit table headers AND inline entries: callers want to highlight both
 * `[tool.poetry.dependencies]\nfoo = "1"` (header keys) and `foo = "1"` inside
 * it. The old toml-eslint-parser traversal did the same thing; we match it.
 */
