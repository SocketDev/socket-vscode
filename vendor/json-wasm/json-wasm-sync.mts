 ...
 *
 * Reads `./json.wasm` from disk synchronously at module-load time via
 * fs.readFileSync + new WebAssembly.Module + new WebAssembly.Instance. No async
 * init, no top-level await.
 *
 * Pairs with json.wasm in the same directory. Mirrors the acorn-wasm vendor's
 * loader shape so esbuild bundling, vsce packaging, and runtime resolution all
 * work the same way.
 */
