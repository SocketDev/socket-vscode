...
export function parseTrustList(output: string): TrustBinding {
  ...
  for (let i = 0, { length } = lines; i < length; i += 1) {
    ...
    // oxlint-disable-next-line socket/require-regex-comment -- key
...
export default async function main(): Promise<void> {
  ...
  for (let i = 0, { length } = packages; i < length; i += 1) {
    ...
    // Npm's trust docs ask for serial, spaced reads.
    // eslint-disable-next-line no-await-in-loop -- npm's trust docs ask
