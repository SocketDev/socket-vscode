 ...
 *   scripts/fleet/check/commits-carry-no-ai-attribution.mts [--unpushed]
 */
...
export function formatFindings(scan: AttributionScan): string {
  const lines: string[] = [
    '[commits-carry-no-ai-attribution] AI-attribution fingerprint(s) found.',
    '',
...
export function formatCleanSummary(scan: AttributionScan): string {
  return `[commits-carry-no-ai-attribution] ok — ${scan.commitsScanned} commit(s) (${scan.scope}) and ${scan.branchesScanned} branch(es) carry no AI attribution.`
}
...
export async function runCheck(
  ...
  try {
    ...
    logger.fail(
      [
        '[commits-carry-no-ai-attribution] cannot verify the AI-attribution rule.',
        '',
