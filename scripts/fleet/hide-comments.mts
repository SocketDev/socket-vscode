 ...
 *
 *   Usage: node scripts/fleet/hide-comments.mts <owner>/<repo> <pr> [--dry-run] [--reason <name>] [--user <login>]
 */
 ...
 */
/**
 * GitHub's own "reason for hiding this comment" menu, verbatim. The API name
 * for each is the uppercase form, and `minimizeComment` rejects anything else,
 * so an unknown `--reason` fails here rather than as an opaque GraphQL error.
 */
...
/**
 * Normalize a `--reason` to its API spelling, or undefined when it names no
 * reason GitHub offers.
 */
export function parseHideReason(value: string): HideReason | undefined {
  const upper = value
    .trim()
    .toUpperCase()
    .replace(/[\s-]+/g, '_')
  return (HIDE_REASONS as readonly string[]).includes(upper)
    ? (upper as HideReason)
    : undefined
}
...
async function minimizeComment(
  subjectId: string,
  classifier: HideReason = 'RESOLVED',
): Promise<void> {
  await gh([
    'api',
    'graphql',
    '-f',
    `query=mutation { minimizeComment(input: {subjectId: "${subjectId}", classifier: ${classifier}}) { minimizedComment { isMinimized } } }`,
  ])
}
...
function surfaceEntries(
  nodes: readonly RawSurfaceNode[],
  users: readonly string[] = [],
): SurfaceEntry[] {
  // Bots are the default target because their review noise is what buries a
  // thread. A named `--user` opts specific humans in; nothing hides a human
  // by default, since a person's comment disappearing without being asked for
  // is worse than noise.
  const { bot, human } = partitionBotSurfaces(nodes)
  const wanted = new Set(users.map(u => u.toLowerCase()))
  const selected = wanted.size
    ? [
        ...bot,
        ...human.filter(n => wanted.has((n.author?.login ?? '').toLowerCase())),
      ]
    : bot
...
export async function main(): Promise<number> {
  ...
  const reason =
    typeof reasonArg === 'string' ? parseHideReason(reasonArg) : 'RESOLVED'
  if (!reason) {
    logger.fail(
      `unknown --reason ${JSON.stringify(reasonArg)}.\n` +
        '  What: GitHub only accepts the reasons on its own hide menu.\n' +
        '  Where: the --reason flag.\n' +
        `  Saw vs. wanted: ${JSON.stringify(reasonArg)}; wanted one of ${HIDE_REASONS.join(', ').toLowerCase()}.\n` +
        '  Fix: pass one of those, or omit --reason for resolved.',
    )
    return 1
  }
  const users = Array.isArray(values['user'])
  ...
  if (
    ...
    logger.fail(
      'usage: node scripts/fleet/hide-comments.mts <owner>/<repo> <pr-number> [--dry-run] [--reason <name>] [--user <login>]\n' +
  ...
  const surfaces = surfaceEntries(
    [...prData.reviews.nodes, ...prData.comments.nodes],
    users,
  )

  if (dryRun) {
    logger.log(`[hide-comments] plan for ${ownerRepo}#${pr}:`)
    logger.log(
  ...
  for (const t of threads) {
    try {
      ...
      logger.warn(
        `[hide-comments] resolveReviewThread(${t.id}) failed: ${errorMessage(e)}`,
      )
    }
  }
  ...
  for (const entry of surfaces) {
    ...
    try {
      await minimizeComment(entry.node.id, reason)
      results.push({
        id: entry.node.id,
        kind: entry.kind,
        login: entry.login,
  ...
  if (remaining.length > 0) {
    ...
    for (let i = 0, { length } = remaining; i < length; i += 1) {
      ...
      if (action?.kind === 'post') {
        ...
        logger.fail(`[hide-comments] ${needsUiMessage(ownerRepo, pr, entry)}`)
      }
    }
  }

  logger.log(`[hide-comments] ${ownerRepo}#${pr} report:`)
  ...
  if (verdict === 0) {
    logger.success(
      `[hide-comments] ${ownerRepo}#${pr}: every bot surface is minimized or has a directive posted.`,
    )
  }
  return verdict
}
...
const SCRIPT_META: ScriptMeta = {
  describe:
    "hides a PR's review noise: resolves threads and minimizes comments with a reason",
  help: `Usage: node scripts/fleet/hide-comments.mts <owner>/<repo> <pr-number> [flags]

  --dry-run        print the plan without mutating anything
  --reason <name>  why the comments are hidden: abuse, duplicate, low-quality,
                   off-topic, outdated, resolved, spam (default: resolved)
  --user <login>   also hide this person's comments (repeatable). Bots are
                   hidden by default; a human is only ever hidden by name.`,
}
