...
// Code/test file extension: `.json` or an optional-`m`, `j`/`t`, optional-`sx`
// source extension (`.js`, `.mjs`, `.jsx`, `.ts`, `.mts`, `.tsx`, …).
const CODE_FILE_EXT_RE = /\.(?:json|m?[jt]sx?)$/
// Test-shaped path: a `__tests__`/`test`/`tests` path segment, or a
// `.test.`/`.spec.` filename infix.
const TEST_PATH_SHAPE_RE =
...
export function checkFeatureParity(
  ...
  const codeFiles = walkDirFiles(localAreaPath, CODE_FILE_EXT_RE).filter(
    f => !TEST_PATH_SHAPE_RE.test(f),
  )
  ...
  const testAreaFiles = walkDirFiles(testAreaPath, CODE_FILE_EXT_RE)
  const testFiles = row.test_area
    ? testAreaFiles
    : testAreaFiles.filter(f => TEST_PATH_SHAPE_RE.test(f))
  const testScore =
