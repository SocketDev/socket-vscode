...
export const scanWorkflowFile = (repoRoot: string, relPath: string): void => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0, { length } = matches; j < length; j += 1) {
  ...
  for (const [pathStr, hits] of occurrences) {
    ...
    for (let j = 0, { length } = hits; j < length; j += 1) {
      const hit = hits[j]!
      pushFinding({
        rule: 'C',
        file: relPath,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // build/<dev|prod|shared>/<pkg-name>/[wasm/]out/<Final-stage-name> — the
    // fully-qualified build output path shape owned by paths.mts.
    const literalShape =
