...
// YAML list entry `- [@scope/]name@version` — captures (1) the optionally
// scoped package name, (2) the version/range after the last `@`.
const ENTRY_RE =
  /^\s*-\s*['"]?((?:@[^@/'"\s]+\/)?[^@'"\s]+)@([^'"\s]+)['"]?\s*$/
const GLOB_ENTRY_RE = /^\s*-\s*['"]?[^'"\s]*\*[^'"\s]*['"]?\s*$/
const BARE_NAME_ENTRY_RE = /^\s*-\s*['"]?[^@'"\s]+['"]?\s*$/
// Soak annotation comment `# published: YYYY-MM-DD | removable: YYYY-MM-DD`
// — captures (1) published date, (2) removable date.
const ANNOTATION_RE =
