// max-file-lines: table — evaluate() is a flat, sequential GitHub-settings rule table; each check is an independent `check(...)` call against one canonical settings payload, and splitting the table across files would scatter cohesive audit logic without reducing complexity.
/**
