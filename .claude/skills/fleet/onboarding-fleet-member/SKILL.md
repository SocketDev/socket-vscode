...
description: Bring a repo fully into the Socket fleet: config, tooling, hooks, security, and landing.
user-invocable: true
...
Parse the repo's REALITY, not assumptions. Run the detector:

```bash
# socket-lint: allow cross-repo
node scripts/repo/detect-fleet-member.mts <repo-name|path> --json
```

It emits `{ repoName, packageName, private, hasBin, repo.type, build, capabilities,
antiFleetDeps, antiFleetConfigFiles, hasRolldownConfig, defaultBranch, bundledOutput }`
— every field a pure read (it wraps the `fleet-members-are-onboarded.mts` collectors, so
the directory conventions live in one owner). It reads package.json (not regex) and
config-file EXISTENCE (not string-grep — `eslint`/`esbuild`/`biome` appear in fleet rule
text and false-positive a grep). Crucially, `build.from` AND `build.type` come back `null`
with `buildFromAmbiguous: true` — the npm-registry-vs-github-release call and the
js/addon/binary call are NOT inferable, so the detector leaves `build.{from,type}` absent
and sets `buildFromAmbiguous: true` — the flag YOU key on to ASK rather than guess (the
judgment seam). `capabilities.cargo` lists the crate dirs from tracked non-fixture
`.rs`/Cargo.toml; it never sets `build.type` (cargo is orthogonal to build).
...
description, optIns? }`. The `repos` array follows a deliberate GROUPING (socket-\*
   members alphabetically, then the bare-prefix members sdxgen/stuie/ultrathink, then
   socket-wheelhouse last) — NOT a flat alpha sort; insert the new entry into its group,
   don't re-sort the whole array — a naive sort scrambles the grouping. ASK the user
   whether the repo is `squash-history` opt-in (currently socket-addon/bin/btm/sdxgen/stuie).
...
6. **Scaffolding cascade.** In the wheelhouse, run `pnpm -C <wheelhouse> run onboard -- --target <repo>` (which backs up,
...
13. **CLAUDE.md.** In the wheelhouse, run `node <wheelhouse>/scripts/repo/migrate-claude-md.mts --target <repo> --apply`
    ...
    the `<fleet-canonical>` open/close markers (`<!-- <fleet-canonical> -->` … `<!-- </fleet-canonical> -->`) around the fleet block.
...
18. **CI + local-CI verification.** Three pieces, all cascaded — confirm they landed:
    ...
    - **The gated local-CI test** — `test/repo/unit/agent-ci-local.test.mts` cascades via
      the `test/repo/unit` dir-mirror. It asserts the `ci:local` script keeps its
...
19. **Verify GREEN.** `pnpm install`, then `pnpm run check --all` + `pnpm test` +
    ...
    `node <wheelhouse>/scripts/repo/check/fleet-members-are-onboarded.mts` (from the wheelhouse) —
...
- Cascade: the wheelhouse `onboard` script (`pnpm -C <wheelhouse> run onboard`) / the `cascading-fleet` skill.
