...
**Why the fleet should borrow this:** today every fleet repo carries 200–450-line `scripts/repo/check.mts` / `scripts/fix.mts` / `scripts/fleet/test.mts` files that duplicate "find the tool binary, build the right args, exec it." Real drift surface — the same logic written 12 times rarely stays in sync.
...
```ts
// Caller (per-repo scripts/repo/check.mts):
import {
  resolveLinter,
  runResolved,
