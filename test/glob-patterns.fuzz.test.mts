...
const nonLetterString = fc
  .array(fc.constantFrom(...NON_LETTER_CHARS), { maxLength: 30 })
  .map(cs => cs.join(''))
...
const fileWord = fc
  .array(fc.constantFrom(...FILE_CHARS), { minLength: 1, maxLength: 16 })
  .map(cs => cs.join(''))
...
describe('data/glob-patterns replaceCasedChars (fuzz)', () => {
  ...
  test('rewrites a single ASCII letter to its case class', () => {
    fc.assert(
      fc.property(fc.constantFrom(...LETTERS), c => {
  ...
  test('rewritten word matches any-case variants (path.matchesGlob)', () => {
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom(...LOWER), { minLength: 1, maxLength: 12 }),
...
describe('data/glob-patterns caseDesensitize (fuzz)', () => {
  ...
  test('preserves an existing character group verbatim', () => {
    const groupInner = fc
      .array(
        fc.constantFrom(...LETTERS, ...NON_LETTER_CHARS.replace(']', '')),
        {
          minLength: 1,
          maxLength: 10,
        },
  ...
  test('equals replaceCasedChars when there is no character group', () => {
    const noBracket = fc
      .array(fc.constantFrom(...LETTERS, ...NON_LETTER_CHARS), {
        maxLength: 30,
      })
      .map(cs => cs.join(''))
    fc.assert(
