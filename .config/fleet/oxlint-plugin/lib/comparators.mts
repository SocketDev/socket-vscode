 ...
 *   alphanumeric order identical across every sort surface.
 *
 *   The order is the fleet's canonical **natural** sort, delegated to
 *   `@socketsecurity/lib`'s `naturalCompare`: case-insensitive and
 *   numeric-aware, so `apple, Mango, zebra` (not ASCII `Mango, apple, zebra`)
 *   and `item1, item2, item10` (not `item1, item10, item2`). Both primitives
 *   share the one comparator so the "already sorted" fast-path can never
 *   disagree with the sorter it guards.
 */

import { naturalCompare } from '@socketsecurity/lib-stable/sorts/natural'

/**
 * Total order over two strings: the fleet's natural comparator (case-insensitive
 * + numeric-aware) from `@socketsecurity/lib`. Pass extracted sort keys, not
 * nodes.
 */
