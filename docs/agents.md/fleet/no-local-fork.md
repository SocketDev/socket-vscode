...
Fleet-canonical files (anything tracked by the wheelhouse's `scripts/sync-scaffolding/manifest.mts`) MUST be edited in the wheelhouse `template/...` and cascaded out. Never branched locally in a downstream fleet repo.
...
If unsure, check the wheelhouse's `scripts/sync-scaffolding/manifest.mts`. Tracked = canonical.
